...
* Add ``stor_s3.S3Path.restore(days, tier)`` to restore *single* objects from Glacier to S3.
...
* Move ``ConflictError`` to ``stor.exceptions`` (still available under ``stor_swift.swift``)
