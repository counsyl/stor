...
class TestDiv(unittest.TestCase):
    def test_success(self):
        p = old_div(windows.WindowsPath(r'my\path'), r'other\path')
    ...
    def test_w_posix_path(self):
        with self.assertRaisesRegexp(TypeError, 'unsupported operand'):
            old_div(windows.WindowsPath(r'my\path'), posix.PosixPath('other/path'))

    def test_w_swift_component(self):
        with self.assertRaisesRegexp(TypeError, 'unsupported operand'):
            old_div(windows.WindowsPath(r'my\path'), swift.SwiftPath('swift://t/c/name').name)

    def test_w_s3_component(self):
        with self.assertRaisesRegexp(TypeError, 'unsupported operand'):
            old_div(windows.WindowsPath(r'my\path'), s3.S3Path('s3://b/name').name)
